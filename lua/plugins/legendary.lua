  ...
  config = function()
    ...
    require("legendary").setup({
      ...
      keymaps = {
        ...
        -- ╰──────────────────────────────────────────────────────────╯
        -- general
        { "<leader>MXGa", "<cmd>ScopeMoveBuf<CR>", description = "Scope: Move Current Buffer to Specified Tab" },
        { "<leader>MXGb", function() vim.api.nvim_feedkeys(":verbose map <C-i>", "c", false) end, description = "Find Keybinding Conflict" },
        { "<leader>MXGc", function() require("core.utils").markdown_preview() end, description = "Preview Markdown" },
        { "<leader>MXGd", "<cmd>ScrollViewToggle<CR>", description = "ScrollViewToggle: Enable" },
        { "<leader>MXGe", "<cmd>TSContextToggle<CR>", description = "TSContextToggle: Toggle" },
        { "<leader>MXGf", "<cmd>Telescope notify<Esc>", description = "Notify: Search History" },

        -- Lazy
        { "<leader>MXla", "<cmd>Lazy sync<CR>", description = "Lazy: Update" },
        { "<leader>MXlb", "<cmd>Lazy clean<CR>", description = "Lazy: Clean" },

        -- LSP info
        { "<leader>MXlc", "<cmd>LspInfo<CR>", description = "LSP: Info" },
        { "<leader>MXld", "<cmd>Mason<CR>", description = "Mason: Info" },
        { "<leader>MXle", "<cmd>MasonToolsUpdate<CR>", description = "Mason: Tool Update" },

        -- persisted.nvim
        { "<leader>MXPa", "<cmd>SessionSave<CR>", description = "Session: Save" },
        { "<leader>MXPb", "<cmd>SessionLoad<CR>", description = "Session: Load" },
        { "<leader>MXPc", "<cmd>SessionLoadLast<CR>", description = "Session: Load Recent Session" },
        { "<leader>MXPd", "<cmd>SessionDelete<CR>", description = "Session: Delete" },

        -- global commands
        { "<leader>MXgga", function() vim.api.nvim_feedkeys(":g/^$/d", "c", false) end, description = "g: Remove Empty Lines", },
        { "<leader>MXggb", function() vim.api.nvim_feedkeys(":g/^foo$/d", "c", false) end, description = "g: Delete Lines Only Contain foo", },
        { "<leader>MXggc", function() vim.api.nvim_feedkeys(":g!/^foo$/d", "c", false) end, description = "g: Delete Lines NOT foo", },
        { "<leader>MXgge", function() vim.api.nvim_feedkeys(":g/foo/normal! A;", "c", false) end, description = "g: Run Normal Mode Containing foo", },
        { "<leader>MXggf", function() vim.api.nvim_feedkeys(":g/foo/normal! @a", "c", false) end, description = "g: Run Macro Containing foo", },
        { "<leader>MXggh", function() vim.api.nvim_feedkeys(":g/foo/t $", "c", false) end, description = "g: t (copy) foo to End of File", },
        { "<leader>MXggi", function() vim.api.nvim_feedkeys(":g/foo/m $", "c", false) end, description = "g: (m)ove foo to End of File", },

        -- substitute
        { "<leader>MXselc", function() vim.api.nvim_feedkeys(":sno/foo/bar/gc", "c", false) end, description = "Substitute: (No Magic Mode) substitute `foo` with `bar`", },
        { "<leader>MXrts", "<cmd>%s/\t/  /g<CR>", description = "Convert Tabs to Spaces", },
        { "<leader>MXttes", "<cmd>'<,'>s/ .*//gc<CR>", description = "Delete Everything After a Space", mode = { "x" }},
        { "<leader>MXraw", "<cmd>%s/\\s\\+$//e<CR>", description = "Remove All Whitespace"},
        { "<leader>MXrsq", "<cmd>%s/[’‘]/'/g<CR>", description = "Replace All Single Curly Quotes"},
        { "<leader>MXrdq", '<cmd>%s/[“”]/"/g<CR>', description = "Replace All Double Curly Quotes"},
        { "<leader>MXrqq", '<cmd>%s/0 " "//g<CR>', description = "hledger: Remove Zero Quote Quote", filters = { filetype = "ledger" }},
        { "<leader>MXsel", function() vim.api.nvim_feedkeys(":s/$/foo", "c", false) end, description = "Visual Mode: Add `foo` to End Of Line", mode = { "v" } },
        { "<leader>MXsela", function() vim.api.nvim_feedkeys(":s/\\vfoo/&bar/gc", "c", false) end, description = "Visual Mode: `foo` into `foobar`", mode = { "v" } },
        { "<leader>MXselb", function() vim.api.nvim_feedkeys(":s/\\v(foo)(.*)(bar)/\\3\\2\\1/gc", "c", false) end, description = "Visual Mode: swap `foo` with `bar`", mode = { "v" } },

        -- leetcode
        { "<leader>MXLm", "<cmd>Leet Menu<CR>", description = "LeetCode: Opens Menu Dashboard" },
        { "<leader>MXLc", "<cmd>Leet console<CR>", description = "LeetCode: Opens Console For Currently Opened Question" },
        { "<leader>MXLq", "<cmd>Leet tabs<CR>", description = "LeetCode: Opens A Picker With All Currently Opened Question Tabs" },
        ...
        -- vim-caser
        { "<leader>MXca", "<Plug>CaserVUpperCase", description = "Constant case: LOREM_IPSUM", mode = { "v" } },
        { "<leader>MXcb", "<Plug>CaserVSnakeCase", description = "Snake case: lorem_ipsum", mode = { "v" } },
        { "<leader>MXcc", "<Plug>CaserVKebabCase", description = "Dash case: lorem-ipsum", mode = { "v" } },
        { "<leader>MXcd", "<Plug>CaserVSpaceCase", description = "space case: lorem ipsum", mode = { "v" } },
        { "<leader>MXce", "<Plug>CaserVDotCase", description = "Dot case: lorem.ipsum", mode = { "v" } },
        { "<leader>MXcf", "<Plug>CaserVSentenceCase", description = "Sentence case: Lorem ipsum", mode = { "v" } },
        { "<leader>MXcg", "<Plug>CaserVMixedCase", description = "Pascal case: LoremIpsum", mode = { "v" } },
        { "<leader>MXch", "<Plug>CaserVTitleCase", description = "Title case: Lorem Ipsum", mode = { "v" } },
        { "<leader>MXci", "<Plug>CaserVTitleKebabCase", description = "Title dash case: Lorem-Ipsum", mode = { "v" } },
        { "<leader>MXcj", "<Plug>CaserVCamelCase", description = "Camel case: loremIpsum", mode = { "v" } },

        -- jdtls
        { "<leader>MXja", "<cmd>JdtWipeDataAndRestart<CR>", filters = { filetype = "java" }, description = "jdtls: Wipe Data and Restart" },
        { "<leader>MXjb", "<cmd>JdtUpdateConfig<CR>", filters = { filetype = "java" }, description = "jdtls: Update Config" },
        { "<leader>MXjc", "<cmd>lua require'jdtls'.organize_imports()<CR>", filters = { filetype = "java" }, description = "jdtls: Organize Import" },
        { "<leader>MXjd", "<cmd>lua require('jdtls').extract_variable()<CR>", filters = { filetype = "java" }, description = "jdtls: Extract Variable" },
        { "<leader>MXje", "<cmd>lua require('jdtls').extract_variable(true)<CR>", filters = { filetype = "java" }, description = "jdtls: Extract Variable" },
        { "<leader>MXjf", "<cmd>lua require('jdtls').extract_constant()<CR>", filters = { filetype = "java" }, description = "jdtls: Extract Constant" },
        { "<leader>MXjg", "<Esc><cmd>lua require('jdtls').extract_constant(true)<CR>", filters = { filetype = "java" }, description = "jdtls: Extract Constant" },
        { "<leader>MXjh", "<Esc><cmd>lua require('jdtls').extract_method(true)<CR>", filters = { filetype = "java" }, description = "jdtls: Extract Method" },
      },
    })
  end,
}
